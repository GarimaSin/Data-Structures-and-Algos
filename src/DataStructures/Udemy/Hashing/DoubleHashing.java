...
public class DoubleHashing {
	...
	int secondHashFunction(String x, int M) {
		...
		for (sum = 0, i = 0; i < xlength; i++)
			sum += ch[i];							/** same as Hash func 1 **/
		while (sum > 13)							
			sum = addAllTheDigitsTogether(sum);		/** add all the individual digits and modulo it by Hash table size **/
		return sum % M;
	}
	...
	public void insertKeyInHashTable(String value) {
		...
		if (loadFactor >= 0.75) {
			...
			for (int i = 0; i < hashTable.length; i++) {
					...
					// If there is a collision then we go to the next index and keep going till we reach starting index.
