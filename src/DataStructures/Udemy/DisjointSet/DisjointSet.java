...
/**
 * 
 * Diagrammatically, Each Set has 1 or more nodes, ==> hence disjoint set class has List of nodes ie Line# 14
 * Also, since find() returns the set to which a node belongs, hence Node class has Disjoint set object
 * Union is done on int not sets
 */
public class DisjointSet {
	...
	public static void driver(ArrayList<WeightedNode> nodeList){
		...
		for(int i= 0; i<nodeList.size()-1; i++) {
			...
			firstNode.getSet().printAllNodesOfThisSet();														//since set is not static, to get a set of a node, 
																																			//call the set() with node's object to get that particular's node's set
		...
		if(node1.getSet().equals(node2.getSet())) { //if two nodes are of same set then no union needed
			...
			if(set1.getNodes().size()>set2.getNodes().size()) {
				...
				for(WeightedNode node: nodeSet2) { 
					node.setSet(set1);				/** Remember **/			//node me set add karo
					set1.getNodes().add(node);										//set me node add karo
				}
				return set1;
			}
			else {
