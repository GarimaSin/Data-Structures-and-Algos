...
/**
 * I/D/Search, all use Recursion = Stack
 */
...
public class BinarySearchTreeByLinkedList {
	...
	public BinaryNode deleteNodeOfBST(BinaryNode root, int value) {
		...
		if (value < root.getValue()) {
			root.setLeft(deleteNodeOfBST(root.getLeft(), value));								/** root.set is imp. **/
		} else if (value > root.getValue()) {
			root.setRight(deleteNodeOfBST(root.getRight(), value));								/** root.set is imp. **/
...
//				deleteNodeOfBST(root.getRight(), minNodeForRight.getValue());					/** both lines, 75, 76 are fine **/
				root.setRight(deleteNodeOfBST(root.getRight(), minNodeForRight.getValue()));  	/** --> root.set <--   Deleting minimum node from right now **/
	...
	void reverseInorder(BinaryNode node) {
		if (node == null) {
			return;
		}
	...
	boolean printAncestors(BinaryNode node, int target)  { 
        ...
        if(target < node.getValue())
        	left = printAncestors(node.getLeft(), target);
        else
        	right = printAncestors(node.getRight(), target);
        if (left || right)  { 
            System.out.print(node.getValue() + " "); 
            return true; 
        } 
