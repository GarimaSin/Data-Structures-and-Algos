package Problem_Solving.DP.Udemy.ConvertOneStringToAnother;
...
public class ConvertOneStringToAnother_TD {
	  ...
	  private int findMinOperationsRecursive(Integer[][] dp, String s1, String s2, int i1, int i2) {
	    ...
	    if(dp[i1][i2] == null) {	//if dp[] has not the value 
	      ...
	      if(i1 == s1.length()) // if we have reached the end of s1, then insert all the remaining characters of s2
	        ...
	        int c1 = 1+ findMinOperationsRecursive(dp, s1, s2, i1+1, i2); //delete
	        int c2 = 1+ findMinOperationsRecursive(dp, s1, s2, i1, i2+1); //insert
	        int c3 = 1+ findMinOperationsRecursive(dp, s1, s2, i1+1, i2+1); //replace
	  ...
	  public static void main(String[] args) {
	    ...
	    System.out.println(editDisatnce.findMinOperations("ABCDFGHJQZ", "ABCDEFGIJKRXYZ"));
	  }
