...
/**
 * We can start by calculating the sum of all elements in the set. If sum is not divisible by k, 
 * we can�t divide the array into k subsets with equal sum. If sum is divisible by k, we check if k subsets 
 * with sum of elements equal to sum/k exists or not
 *
 *
 *	Mind - loop for recursion runs for the length of subset not for the i/p array.
 *			Rather, i/p array's index is managed by recursive call.
 */
public class AllKPartitionsEqualSum {
	// Function to check if all subsets are filled or not
		...
		public static void partition(int[] S, int k)	{
			...
			// create an array of size k for each subset and initialize it by their expected sum i.e. sum/k
			int[] sumLeft = new int[k];
			Arrays.fill(sumLeft, sum/k);

			// return true if sum is divisible by k and the set S can be divided into k subsets with equal sum
