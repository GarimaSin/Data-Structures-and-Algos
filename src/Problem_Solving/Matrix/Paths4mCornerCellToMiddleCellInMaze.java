...
/**
 * Given a square maze containing positive numbers, find all paths from a corner cell (any of the extreme four corners) to the 
 * middle cell. We can move exactly n steps from a cell in 4 directions i.e. North, East, West and South where n is value of 
 * the cell. We can move to mat[i+n][j], mat[i-n][j], mat[i][j+n], and mat[i][j-n] from a cell mat[i][j] 
 * where n is value of mat[i][j].
 * 
 * https://www.geeksforgeeks.org/find-paths-from-corner-cell-to-middle-cell-in-maze/
 */
public class Paths4mCornerCellToMiddleCellInMaze {
	...
	public static void printPath(int[][] maze, int i, int j, String ans){
		...
		if (i == maze.length/2 && j==maze.length/2){
			...
			// Make the final answer, Print the final answer and Return
			ans += "("+i+", "+j+") -> MID";
		...
		// If element is non-zero, then note the element in variable 'k'
		int k = maze[i][j];
