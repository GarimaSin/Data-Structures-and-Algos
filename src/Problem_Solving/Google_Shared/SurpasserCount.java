...
public class SurpasserCount {
							...
							int high, Map<Integer, Integer> count)
	{
		...
		while (i <= mid && j <= high) {
			if (arr[i] > arr[j]) {
				count.put(arr[i], getCount(count, arr[i]) + c);		/** Diff than Merge, put the count in map I - if a[i]>a[j], then no surpass**/
				aux[k++] = arr[i++];
			}
			else {
				aux[k++] = arr[j++];
				c++;							/** I - if a[i]<a[j], then only surpass will happen, hence c++ (opp. of inversion count) **/
			}
		}

		// copy remaining elements
		while (i <= mid) {
			count.put(arr[i], getCount(count, arr[i]) + c);				// surpass code
			aux[k++] = arr[i++];
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < A.length; i++) {
			System.out.print(surpasserCount.get(A[i]) + " ");
		}
	}
}
